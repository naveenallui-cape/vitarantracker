...
export default function DeviceDetailPage() {
  ...
  async function load() {
    const [nextDevice, nextHistory] = await Promise.all([
      api<Device>(`/admin/devices/${params.id}`),
      api<DeviceAssignment[]>(`/admin/devices/${params.id}/history`),
    ]);
  ...
  async function revoke() {
    await api(`/admin/devices/${params.id}/revoke`, { method: "POST" });
    await load();
  }
  ...
  async function replace() {
    ...
    }>(`/admin/devices/${params.id}/replace`, { method: "POST" });
      ...
      {showReassign ? (
        <ReassignModal
          ...
          onSubmit={async (newEmployeeId, reason) => {
            const result = await api<{
              registrationCode: { code: string; expiresAt: string };
            }>(`/admin/devices/${params.id}/reassign`, {
              method: "POST",
