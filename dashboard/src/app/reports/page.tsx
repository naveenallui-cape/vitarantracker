...
export default function ReportsPage() {
  const [report, setReport] = useState<WorkTimeReport | null>(null);
  ...
  async function load(event: FormEvent<HTMLFormElement>) {
    ...
    for (const key of ["from", "to", "employeeId", "department", "deviceId"]) {
  ...
  async function exportCsv(form: HTMLFormElement) {
    ...
    link.href = url;
    link.download = "work-time-report.csv";
    link.click();
  ...
  return (
    <AppShell>
      <h1 className="text-3xl font-semibold">Work-time reports</h1>
      <p className="mt-2 mb-6 max-w-2xl text-sm text-[#5d6b63]">
        Built from daily summaries, not a login-minus-logout calculation. This
        is computer activity on company-owned Windows laptops.
      </p>
      <form
        onSubmit={(event) => void load(event)}
        className="mb-6 grid gap-3 rounded-2xl bg-[var(--panel)] p-5 sm:grid-cols-3"
      >
        <input name="from" type="date" className="rounded-lg border border-[#d9d4c8] px-3 py-2 text-sm" />
        <input name="to" type="date" className="rounded-lg border border-[#d9d4c8] px-3 py-2 text-sm" />
        <input name="employeeId" placeholder="Employee ID" className="rounded-lg border border-[#d9d4c8] px-3 py-2 text-sm" />
        <input name="department" placeholder="Department" className="rounded-lg border border-[#d9d4c8] px-3 py-2 text-sm" />
        <input name="deviceId" placeholder="Device ID" className="rounded-lg border border-[#d9d4c8] px-3 py-2 text-sm" />
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={(event) =>
              void exportCsv(event.currentTarget.form!)
            }
      ...
      {report ? (
        <>
          <p className="mb-3 text-sm text-[#5d6b63]">
            Active {formatDuration(report.totals.activeSeconds)} · Idle{" "}
            ...
            <table className="w-full text-left text-sm">
              <thead className="border-b border-[#d9d4c8] text-xs uppercase text-[#5d6b63]">
                <tr>
                  ...
                  <th className="px-4 py-3">First active</th>
                </tr>
              </thead>
              <tbody>
                {report.rows.map((row) => (
                  <tr key={`${row.employeeId}-${row.date}`} className="border-t border-[#efeae0]">
                    ...
                    <td className="px-4 py-3">{formatTimestamp(row.firstActiveAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
